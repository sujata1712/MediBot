...
# Returns the SQLChatMessageHistory object for a given session.
...
# Wraps a LangChain chain with automatic SQLite message history.
...
# Creates a short, readable title from the user's first message.
...
# Saves an auto-generated title for a session based on the first user message. Uses INSERT OR IGNORE to avoid overwriting an existing title.
...
# Retrieves the title for a session, or returns the session_id if no title is found.
...
# Searches session titles for a keyword (case-insensitive) and returns a list of matching session_id and title pairs, ordered by most recent first.
...
# Returns list of all session_ids ordered by most recent.
